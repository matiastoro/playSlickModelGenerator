...
case class AircraftWakeTurbCategoryFilter(id: Option[Long] = None,
                                          wakeTurbCategory: Option[String] = None)

object AircraftWakeTurbCategoryFilter {
  val tupled = (this.apply _).tupled
}


object AircraftWakeTurbCategoryFilterForm{
  
  val filterForm = Form(
            mapping(
      "id" -> optional(longNumber),
      "wakeTurbCategory" -> optional(nonEmptyText)
    )/*(AircraftWakeTurbCategoryFilter.apply)(AircraftWakeTurbCategoryFilter.unapply)*/
    ((id,wakeTurbCategory) => {
      AircraftWakeTurbCategoryFilter(id, wakeTurbCategory)
    })((formData: AircraftWakeTurbCategoryFilter) => {
      Some((formData.id, formData.wakeTurbCategory))
    })
  )
}
