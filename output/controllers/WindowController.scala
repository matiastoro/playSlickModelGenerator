...
*/
  ...
  def save() = Action{ implicit request =>
    ...
    form.bindFromRequest.fold(
      formWithErrors => {
        ...
        Redirect(routes.WindowController.show(id)).flashing("success" -> Messages("save.success"))
      }
    )
  }
  ...
  def delete(id: Long) = Action{ implicit request =>
    WindowQuery.byId(id).map{ window =>
      ...
      Redirect(routes.WindowController.index(1,20)).flashing("success" -> Messages("delete.success"))
    }.getOrElse(NotFound)
  }
        ...
        formWithErrors => {
         val branch = BranchQuery.getAll
          ...
          formData.update(formData.obj.copy(id = window.id)).map{ id =>
            Redirect(routes.WindowController.show(window.id.get)).flashing("success" -> Messages("save.success"))
          }.getOrElse(NotFound)
        }
      )
    }.getOrElse(NotFound)
