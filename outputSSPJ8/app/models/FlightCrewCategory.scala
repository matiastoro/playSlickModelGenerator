...
case class FlightCrewCategoryFilter(id: Option[Long] = None,
                                    category: Option[String] = None)

object FlightCrewCategoryFilter {
  val tupled = (this.apply _).tupled
}


object FlightCrewCategoryFilterForm{
  
  val filterForm = Form(
            mapping(
      "id" -> optional(longNumber),
      "category" -> optional(nonEmptyText)
    )/*(FlightCrewCategoryFilter.apply)(FlightCrewCategoryFilter.unapply)*/
    ((id,category) => {
      FlightCrewCategoryFilter(id, category)
    })((formData: FlightCrewCategoryFilter) => {
      Some((formData.id, formData.category))
    })
  )
}
