...
case class LanguageFilter(id: Option[Long] = None,
...
object LanguageFilterForm{
  
  val filterForm = Form(
            mapping(
      "id" -> optional(longNumber),
      "language" -> optional(nonEmptyText)
    )/*(LanguageFilter.apply)(LanguageFilter.unapply)*/
    ((id,language) => {
      LanguageFilter(id, language)
    })((formData: LanguageFilter) => {
      Some((formData.id, formData.language))
    })
  )
}
