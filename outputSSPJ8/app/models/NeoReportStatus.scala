...
case class NeoReportStatusFilter(id: Option[Long] = None,
                                 reportStatus: Option[String] = None)

object NeoReportStatusFilter {
  val tupled = (this.apply _).tupled
}


object NeoReportStatusFilterForm{
  
  val filterForm = Form(
            mapping(
      "id" -> optional(longNumber),
      "reportStatus" -> optional(nonEmptyText)
    )/*(NeoReportStatusFilter.apply)(NeoReportStatusFilter.unapply)*/
    ((id,reportStatus) => {
      NeoReportStatusFilter(id, reportStatus)
    })((formData: NeoReportStatusFilter) => {
      Some((formData.id, formData.reportStatus))
    })
  )
}
