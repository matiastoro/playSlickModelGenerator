...
*/
  ...
  def save() = Action{ implicit request =>
    ...
    form.bindFromRequest.fold(
      formWithErrors => {
        ...
        Redirect(routes.CompanyController.show(id)).flashing("success" -> Messages("save.success"))
      }
    )
  }
  ...
  def delete(id: Long) = Action{ implicit request =>
    CompanyQuery.byId(id).map{ company =>
      ...
      Redirect(routes.CompanyController.index(1,20)).flashing("success" -> Messages("delete.success"))
    }.getOrElse(NotFound)
  }
      ...
      form.bindFromRequest.fold(
        formWithErrors => {
          ...
          formData.update(formData.obj.copy(id = company.id, createdAt = company.createdAt)).map{ id =>
            Redirect(routes.CompanyController.show(company.id.get)).flashing("success" -> Messages("save.success"))
          }.getOrElse(NotFound)
        }
      )
    }.getOrElse(NotFound)
