...
case class OperationTypeFilter(id: Option[Long] = None,
                               operationType: Option[String] = None,
                               operationTypeId: Option[Long] = None)

object OperationTypeFilter {
  val tupled = (this.apply _).tupled
}


object OperationTypeFilterForm{
  
  val filterForm = Form(
            mapping(
      "id" -> optional(longNumber),
      "operationType" -> optional(nonEmptyText),
      "operationTypeId" -> optional(longNumber)
    )/*(OperationTypeFilter.apply)(OperationTypeFilter.unapply)*/
    ((id,operationType,operationTypeId) => {
      OperationTypeFilter(id, operationType, operationTypeId)
    })((formData: OperationTypeFilter) => {
      Some((formData.id, formData.operationType, formData.operationTypeId))
    })
  )
}
