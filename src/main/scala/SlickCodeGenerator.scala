...
object parser {
  ...
  def writeToFile(path: String, submoduleName: String, table: Table, tablesOneToMany: List[Table], buildType: String, tables: List[Table])(implicit langHash: Map[String,String]) = {
    ...
    if(buildType == "all" || buildType == "react"){
      ...
      Files.write(Paths.get(path + "/react/" + submodulePath + table.viewsPackage +  s"/${table.className}Form.jsx"), ReactFormGenerator(table, tablesOneToMany, submodulePackageString, tables).generate.getBytes(StandardCharsets.UTF_8))
