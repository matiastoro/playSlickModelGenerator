...
case class AircraftPropulsionTypeFilter(id: Option[Long] = None,
                                        propulsionType: Option[String] = None)

object AircraftPropulsionTypeFilter {
  val tupled = (this.apply _).tupled
}


object AircraftPropulsionTypeFilterForm{
  
  val filterForm = Form(
            mapping(
      "id" -> optional(longNumber),
      "propulsionType" -> optional(nonEmptyText)
    )/*(AircraftPropulsionTypeFilter.apply)(AircraftPropulsionTypeFilter.unapply)*/
    ((id,propulsionType) => {
      AircraftPropulsionTypeFilter(id, propulsionType)
    })((formData: AircraftPropulsionTypeFilter) => {
      Some((formData.id, formData.propulsionType))
    })
  )
}
