...
case class DetectionPhaseFilter(id: Option[Long] = None,
                                detectionPhase: Option[String] = None)

object DetectionPhaseFilter {
  val tupled = (this.apply _).tupled
}


object DetectionPhaseFilterForm{
  
  val filterForm = Form(
            mapping(
      "id" -> optional(longNumber),
      "detectionPhase" -> optional(nonEmptyText)
    )/*(DetectionPhaseFilter.apply)(DetectionPhaseFilter.unapply)*/
    ((id,detectionPhase) => {
      DetectionPhaseFilter(id, detectionPhase)
    })((formData: DetectionPhaseFilter) => {
      Some((formData.id, formData.detectionPhase))
    })
  )
}
