...
*/
  def index(page: Int = 1, pageLength: Int = 20) = Action { implicit request =>
    val pagination = InspeccionQuery.paginate(InspeccionQuery.allQuery,pageLength,page)
    Ok(views.html.inspeccion.index(pagination.results, pagination.count, page, pageLength))
  }

  def show(id: Long) = Action{ implicit request =>
    InspeccionQuery.byId(id).map{ inspeccion =>
  ...
  def delete(id: Long) = Action{ implicit request =>
    InspeccionQuery.byId(id).map{ inspeccion =>
      InspeccionQuery.delete(inspeccion)
  ...
  def showByActividad(id: Option[Long]) = {
    views.html.inspeccion._nestedShow(InspeccionQuery.byActividadId(id))
  }
}