...
class UserController @Inject()(cc: MessagesControllerComponents)(implicit repo: UserRepository, armyRepo: ArmyRepository, profileUserRepo: ProfileUserRepository, characterRepo: CharacterRepository, ec: ExecutionContext) extends MessagesAbstractController(cc) with Autorization {
...
*/
    ...
    repo.byId(id).flatMap{ oobj =>
      oobj.map{obj =>
        for{
          ...
          characters <- characterRepo.byUserId(Some(id))
       } yield {
  ...
  def options() = withUserAsync{ user =>  implicit request =>
    for{
      armys <- armyRepo.getAll
      } yield {
        Ok(Json.obj("armys" -> Json.toJson(armys)))
      }
