...
case class OcurrenceCategoryFilter(id: Option[Long] = None,
                                   code: Option[String] = None,
                                   ocurrenceCategory: Option[String] = None)

object OcurrenceCategoryFilter {
  val tupled = (this.apply _).tupled
}


object OcurrenceCategoryFilterForm{
  
  val filterForm = Form(
            mapping(
      "id" -> optional(longNumber),
      "code" -> optional(nonEmptyText),
      "ocurrenceCategory" -> optional(nonEmptyText)
    )/*(OcurrenceCategoryFilter.apply)(OcurrenceCategoryFilter.unapply)*/
    ((id,code,ocurrenceCategory) => {
      OcurrenceCategoryFilter(id, code, ocurrenceCategory)
    })((formData: OcurrenceCategoryFilter) => {
      Some((formData.id, formData.code, formData.ocurrenceCategory))
    })
  )
}
