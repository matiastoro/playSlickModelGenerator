...
class ActividadMapping(tag: Tag) extends Table[Actividad](tag, "actividad") {
...
class ActividadQueryBase extends DatabaseClient[Actividad] {
  type DBTable = ActividadMapping
...
case class ActividadFormData(obj: Actividad, Inspeccions: List[InspeccionFormData]){
  def update(updatedObj: Actividad = obj) = {
    ...
    InspeccionQuery.byActividadId(obj.id).filterNot{o => Inspeccions.exists(_.obj.id == o.id)}.map{InspeccionQuery.delete(_)}
    Inspeccions.map{o => o.update(o.obj.copy(actividadId = obj.id.get))}
    ActividadQuery.updateOrInsert(updatedObj)
  }
  def insert(insertedObj: Actividad) = {
    val id = ActividadQuery.insert(insertedObj)
    Inspeccions.map{o => o.insert(o.obj.copy(actividadId = id))}
    id
  }
}
object ActividadFormData{
  def apply(obj: Actividad) = {
    val Inspeccions = InspeccionQuery.byActividadId(obj.id).map(InspeccionFormData(_))
