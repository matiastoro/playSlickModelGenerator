...
case class NestedFormGenerator(table: Table, tablesOneToMany: List[Table] = List(), submodulePackageString: String = "") extends CodeGenerator{
  ...
  def generate: String = {
    ...
    val html = """@(frm: play.api.data.Field"""+args+""", """+table.objName+"""Opt: Option[models."""+table.className+"""] = None)(implicit flash: Flash, lang: Lang, session: Session, context: controllers"""+submodulePackageString+""".ApplicationContext, request: Request[AnyContent])
@import myHelper._
@import helper._
