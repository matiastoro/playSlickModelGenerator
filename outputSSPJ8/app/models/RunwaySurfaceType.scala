...
case class RunwaySurfaceTypeFilter(id: Option[Long] = None,
                                   surfaceType: Option[String] = None)

object RunwaySurfaceTypeFilter {
  val tupled = (this.apply _).tupled
}


object RunwaySurfaceTypeFilterForm{
  
  val filterForm = Form(
            mapping(
      "id" -> optional(longNumber),
      "surfaceType" -> optional(nonEmptyText)
    )/*(RunwaySurfaceTypeFilter.apply)(RunwaySurfaceTypeFilter.unapply)*/
    ((id,surfaceType) => {
      RunwaySurfaceTypeFilter(id, surfaceType)
    })((formData: RunwaySurfaceTypeFilter) => {
