...
class NeoEventTypeRepository @Inject() (dbConfigProvider: DatabaseConfigProvider)(implicit ec: ExecutionContext) extends DatabaseClient[NeoEventType] with NeoEventTypeQuery{
  ...
  */
  ...
  override val sortMap = Map[String, SortMapType](
    ...
    "eventType" -> {(q,s) => q.sortBy(_.eventType)(s)}
  )
  ...
  */
  ...
  def filter(formData: NeoEventTypeFilter) = {
    all
      .filteredBy( formData.id.flatMap{x => if(x>0L) Some(x) else None} )( _.id === _ )
      .filteredBy(formData.eventType)((x,y) => x.eventType.toUpperCase like ('%'+y.toUpperCase+'%'))
  }
}