...
class NeoFlightOperationEventTypePhaseRepository @Inject() (dbConfigProvider: DatabaseConfigProvider, neo_flight_operationRepo: NeoFlightOperationRepository, neo_event_typeRepo: NeoEventTypeRepository, phaseRepo: PhaseRepository)(implicit ec: ExecutionContext) extends DatabaseClient[NeoFlightOperationEventTypePhase] with NeoFlightOperationEventTypePhaseQuery{
  ...
  */
  ...
  override val sortMap = Map[String, SortMapType](
    ...
    "neoEventTypeId" -> {(q,s) => q.join(neo_event_typeRepo.all).on(_.neoEventTypeId === _.id).sortBy(_._2.eventType)(s).map{_._1}},
  ...
  */
  ...
  def filter(formData: NeoFlightOperationEventTypePhaseFilter) = {
    all
      ...
      .filteredBy( formData.neoEventTypeId.collect{case x if x>0L => x} )( _.neoEventTypeId === _ )
      .filteredBy( formData.phaseId.collect{case x if x>0L => x} )( _.phaseId === _ )
  }
}