...
case class Inspeccion(id: Option[Long] = None /*None*/,
  ...
  def getActividad = ActividadQuery.byId(actividadId)
}



class InspeccionMapping(tag: Tag) extends Table[Inspeccion](tag, "inspeccion") {
...
class InspeccionQueryBase extends DatabaseClient[Inspeccion] {
  type DBTable = InspeccionMapping
...
case class InspeccionFormData(obj: Inspeccion){
  def update(updatedObj: Inspeccion = obj) = {
    ...
    InspeccionQuery.updateOrInsert(updatedObj)
  }
  def insert(insertedObj: Inspeccion) = {
    val id = InspeccionQuery.insert(insertedObj)
