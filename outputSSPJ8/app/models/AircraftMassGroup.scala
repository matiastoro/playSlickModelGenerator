...
case class AircraftMassGroupFilter(id: Option[Long] = None,
                                   massGroup: Option[String] = None)

object AircraftMassGroupFilter {
  val tupled = (this.apply _).tupled
}


object AircraftMassGroupFilterForm{
  
  val filterForm = Form(
            mapping(
      "id" -> optional(longNumber),
      "massGroup" -> optional(nonEmptyText)
    )/*(AircraftMassGroupFilter.apply)(AircraftMassGroupFilter.unapply)*/
    ((id,massGroup) => {
      AircraftMassGroupFilter(id, massGroup)
    })((formData: AircraftMassGroupFilter) => {
      Some((formData.id, formData.massGroup))
    })
  )
}
