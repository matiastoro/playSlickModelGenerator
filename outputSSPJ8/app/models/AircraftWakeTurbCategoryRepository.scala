...
class AircraftWakeTurbCategoryRepository @Inject() (dbConfigProvider: DatabaseConfigProvider)(implicit ec: ExecutionContext) extends DatabaseClient[AircraftWakeTurbCategory] with AircraftWakeTurbCategoryQuery{
  ...
  */
  ...
  override val sortMap = Map[String, SortMapType](
    ...
    "wakeTurbCategory" -> {(q,s) => q.sortBy(_.wakeTurbCategory)(s)}
  )
  ...
  */
  ...
  def filter(formData: AircraftWakeTurbCategoryFilter) = {
    all
      .filteredBy( formData.id.flatMap{x => if(x>0L) Some(x) else None} )( _.id === _ )
      .filteredBy(formData.wakeTurbCategory)((x,y) => x.wakeTurbCategory.toUpperCase like ('%'+y.toUpperCase+'%'))
  }
}