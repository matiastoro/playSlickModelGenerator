...
class PerfilMapeo(tag: Tag) extends Table[Perfil](tag, "perfil") {
...
class PerfilConsultaBase extends BaseDAO[Perfil] {
...
case class PerfilFormData(obj: Perfil){
  def update(updatedObj: Perfil = obj)(implicit session: Session) = {
    ...
    PerfilConsulta.actualizarOInsertar(updatedObj)
  }
  def insert(insertedObj: Perfil)(implicit session: Session) = {
    val id = PerfilConsulta.insertar(insertedObj)
