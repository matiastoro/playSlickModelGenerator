...
case class HelicopterLandingAreaAreaConfigurationFilter(id: Option[Long] = None,
                                                        areaConfiguration: Option[String] = None)

object HelicopterLandingAreaAreaConfigurationFilter {
  val tupled = (this.apply _).tupled
}


object HelicopterLandingAreaAreaConfigurationFilterForm{
  
  val filterForm = Form(
            mapping(
      "id" -> optional(longNumber),
      "areaConfiguration" -> optional(nonEmptyText)
    )/*(HelicopterLandingAreaAreaConfigurationFilter.apply)(HelicopterLandingAreaAreaConfigurationFilter.unapply)*/
    ((id,areaConfiguration) => {
      HelicopterLandingAreaAreaConfigurationFilter(id, areaConfiguration)
    })((formData: HelicopterLandingAreaAreaConfigurationFilter) => {
      Some((formData.id, formData.areaConfiguration))
    })
  )
}
