...
case class FlightRuleFilter(id: Option[Long] = None,
                            flightRule: Option[String] = None)

object FlightRuleFilter {
  val tupled = (this.apply _).tupled
}


object FlightRuleFilterForm{
  
  val filterForm = Form(
            mapping(
      "id" -> optional(longNumber),
      "flightRule" -> optional(nonEmptyText)
    )/*(FlightRuleFilter.apply)(FlightRuleFilter.unapply)*/
    ((id,flightRule) => {
      FlightRuleFilter(id, flightRule)
    })((formData: FlightRuleFilter) => {
      Some((formData.id, formData.flightRule))
    })
  )
}
