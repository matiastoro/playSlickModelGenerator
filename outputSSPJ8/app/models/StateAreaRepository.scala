...
class StateAreaRepository @Inject() (dbConfigProvider: DatabaseConfigProvider)(implicit ec: ExecutionContext) extends DatabaseClient[StateArea] with StateAreaQuery{
  ...
  */
  ...
  override val sortMap = Map[String, SortMapType](
    ...
    "stateArea" -> {(q,s) => q.sortBy(_.stateArea)(s)},
  ...
  */
  ...
  def filter(formData: StateAreaFilter) = {
    all
      .filteredBy( formData.id.flatMap{x => if(x>0L) Some(x) else None} )( _.id === _ )
      .filteredBy(formData.stateArea)((x,y) => x.stateArea.toUpperCase like ('%'+y.toUpperCase+'%'))
      .filteredBy( formData.stateAreaId.flatMap{x => if(x>0L) Some(x) else None} )( _.stateAreaId.getOrElse(0L) === _ )
  }
}