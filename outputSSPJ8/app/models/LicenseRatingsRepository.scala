...
class LicenseRatingsRepository @Inject() (dbConfigProvider: DatabaseConfigProvider)(implicit ec: ExecutionContext) extends DatabaseClient[LicenseRatings] with LicenseRatingsQuery{
  ...
  */
  ...
  override val sortMap = Map[String, SortMapType](
    ...
    "licenseRatings" -> {(q,s) => q.sortBy(_.licenseRatings)(s)}
  )
  ...
  */
  ...
  def filter(formData: LicenseRatingsFilter) = {
    all
      .filteredBy( formData.id.flatMap{x => if(x>0L) Some(x) else None} )( _.id === _ )
      .filteredBy(formData.licenseRatings)((x,y) => x.licenseRatings.toUpperCase like ('%'+y.toUpperCase+'%'))
  }
}