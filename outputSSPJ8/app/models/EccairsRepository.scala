...
class EccairsRepository @Inject() (dbConfigProvider: DatabaseConfigProvider, injury_levelRepo: InjuryLevelRepository, weather_conditionRepo: WeatherConditionRepository, aircraft_categoryRepo: AircraftCategoryRepository, eccairs_ocurrence_categoryRepo: Provider[EccairsOcurrenceCategoryRepository], operation_typeRepo: OperationTypeRepository, operator_typeRepo: OperatorTypeRepository, aircraft_mass_groupRepo: AircraftMassGroupRepository, ocurrence_classRepo: OcurrenceClassRepository, state_areaRepo: StateAreaRepository, operatorRepo: OperatorRepository)(implicit ec: ExecutionContext) extends DatabaseClient[Eccairs] with EccairsQuery{
  ...
  */
  ...
  override val sortMap = Map[String, SortMapType](
    ...
    "registry" -> {(q,s) => q.sortBy(_.registry)(s)},
    ...
    //displayField for field weatherCondition is not defined in YML
  )
  ...
  */
  ...
  def filter(formData: EccairsFilter) = {
    all
      ...
      .filteredBy(formData.number)((x,y) => x.number.toUpperCase like ('%'+y.toUpperCase+'%'))
      .filteredBy( formData.dateFrom )( _.date >= _ )
      .filteredBy( formData.dateTo )( _.date <= _ )
      ...
      .oneToManyFilterfilter(formData.eccairsOcurrenceCategorys)((x,o) => eccairs_ocurrence_categoryRepo.get().filter(o).filter(y => y.eccairsId === x.id).exists)
      ...
      .filteredBy(formData.registry)((x,y) => x.registry.getOrElse("").toUpperCase like ('%'+y.toUpperCase+'%'))
