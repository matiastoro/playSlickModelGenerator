...
case class AerodromeFilter(id: Option[Long] = None,
                           icaoCode: Option[String] = None,
                           ...
                           runways: List[RunwayFilter] = List(),
                           helicopterLandingAreas: List[HelicopterLandingAreaFilter] = List())

object AerodromeFilter {
  val tupled = (this.apply _).tupled
}


object AerodromeFilterForm{
  
  val filterForm = Form(
            mapping(
      "id" -> optional(longNumber),
      "icaoCode" -> optional(nonEmptyText),
      "aerodromeStatusId" -> optional(longNumber),
      "aerodromeTypeId" -> optional(longNumber),
      "latitude" -> optional(nonEmptyText),
      "longitude" -> optional(nonEmptyText),
      "elevationAboveMsl" -> optional(of(doubleFormat)),
      "runways" -> list(models.RunwayFilterForm.filterForm.mapping),
      "helicopterLandingAreas" -> list(models.HelicopterLandingAreaFilterForm.filterForm.mapping)
    )/*(AerodromeFilter.apply)(AerodromeFilter.unapply)*/
    ((id,icaoCode,aerodromeStatusId,aerodromeTypeId,latitude,longitude,elevationAboveMsl,runways,helicopterLandingAreas) => {
      AerodromeFilter(id, icaoCode, aerodromeStatusId, aerodromeTypeId, latitude, longitude, elevationAboveMsl, runways, helicopterLandingAreas)
    })((formData: AerodromeFilter) => {
      Some((formData.id, formData.icaoCode, formData.aerodromeStatusId, formData.aerodromeTypeId, formData.latitude, formData.longitude, formData.elevationAboveMsl, formData.runways, formData.helicopterLandingAreas))
    })
  )
}
