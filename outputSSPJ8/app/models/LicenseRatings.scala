...
case class LicenseRatingsFilter(id: Option[Long] = None,
                                licenseRatings: Option[String] = None)

object LicenseRatingsFilter {
  val tupled = (this.apply _).tupled
}


object LicenseRatingsFilterForm{
  
  val filterForm = Form(
            mapping(
      "id" -> optional(longNumber),
      "licenseRatings" -> optional(nonEmptyText)
    )/*(LicenseRatingsFilter.apply)(LicenseRatingsFilter.unapply)*/
    ((id,licenseRatings) => {
      LicenseRatingsFilter(id, licenseRatings)
    })((formData: LicenseRatingsFilter) => {
      Some((formData.id, formData.licenseRatings))
    })
  )
}
