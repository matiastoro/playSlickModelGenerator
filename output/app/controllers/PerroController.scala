...
*/
  ...
  def save() = Action{ implicit request =>
    ...
    form.bindFromRequest.fold(
      formWithErrors => {
        ...
        Redirect(controllers.routes.PerroController.show(id)).flashing("success" -> Messages("save.success"))
      }
    )
  }
  ...
  def delete(id: Long) = Action{ implicit request =>
    PerroQuery.byId(id).map{ perro =>
      ...
      Redirect(controllers.routes.PerroController.index(1,20)).flashing("success" -> Messages("delete.success"))
    }.getOrElse(NotFound)
  }
      ...
      form.bindFromRequest.fold(
        formWithErrors => {
          ...
          formData.update(formData.obj.copy(id = perro.id)).map{ id =>
            Redirect(controllers.routes.PerroController.show(perro.id.get)).flashing("success" -> Messages("save.success"))
          }.getOrElse(NotFound)
        }
      )
    }.getOrElse(NotFound)
