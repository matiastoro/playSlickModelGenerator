...
case class FlightCrewFilter(id: Option[Long] = None,
                            neoFlightOperationId: Option[Long] = None,
                            categoryId: Option[Long] = None,
                            experienceThis: Option[Double] = None,
                            experienceAll: Option[Double] = None,
                            dutyLast24Hours: Option[Double] = None,
                            restBeforeDuty: Option[Double] = None,
                            flightCrewLicenses: List[FlightCrewLicenseFilter] = List())

object FlightCrewFilter {
  val tupled = (this.apply _).tupled
}


object FlightCrewFilterForm{
  ...
  val filterForm = Form(
      ...
      "dutyLast24Hours" -> optional(of(doubleFormat)),
      "restBeforeDuty" -> optional(of(doubleFormat)),
      "flightCrewLicenses" -> list(models.FlightCrewLicenseFilterForm.filterForm.mapping)
    )/*(FlightCrewFilter.apply)(FlightCrewFilter.unapply)*/
    ((id,neoFlightOperationId,categoryId,experienceThis,experienceAll,dutyLast24Hours,restBeforeDuty,flightCrewLicenses) => {
      FlightCrewFilter(id, neoFlightOperationId, categoryId, experienceThis, experienceAll, dutyLast24Hours, restBeforeDuty, flightCrewLicenses)
    })((formData: FlightCrewFilter) => {
      Some((formData.id, formData.neoFlightOperationId, formData.categoryId, formData.experienceThis, formData.experienceAll, formData.dutyLast24Hours, formData.restBeforeDuty, formData.flightCrewLicenses))
    })
  )
}
