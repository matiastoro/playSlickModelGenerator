...
case class AircraftLandingGearTypeFilter(id: Option[Long] = None,
                                         landingGearType: Option[String] = None,
                                         aircraftLandingGearTypeId: Option[Long] = None)

object AircraftLandingGearTypeFilter {
  val tupled = (this.apply _).tupled
}


object AircraftLandingGearTypeFilterForm{
  
  val filterForm = Form(
            mapping(
      "id" -> optional(longNumber),
      "landingGearType" -> optional(nonEmptyText),
      "aircraftLandingGearTypeId" -> optional(longNumber)
    )/*(AircraftLandingGearTypeFilter.apply)(AircraftLandingGearTypeFilter.unapply)*/
    ((id,landingGearType,aircraftLandingGearTypeId) => {
      AircraftLandingGearTypeFilter(id, landingGearType, aircraftLandingGearTypeId)
    })((formData: AircraftLandingGearTypeFilter) => {
      Some((formData.id, formData.landingGearType, formData.aircraftLandingGearTypeId))
    })
  )
}
