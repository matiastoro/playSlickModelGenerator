...
class AircraftCategoryRepository @Inject() (dbConfigProvider: DatabaseConfigProvider)(implicit ec: ExecutionContext) extends DatabaseClient[AircraftCategory] with AircraftCategoryQuery{
  ...
  */
  ...
  override val sortMap = Map[String, SortMapType](
    ...
    "aircraftCategoryId" -> {(q,s) => q.join(all).on(_.aircraftCategoryId === _.id).sortBy(_._2.category)(s).map{_._1}}
  )
  ...
  */
  ...
  def filter(formData: AircraftCategoryFilter) = {
    all
      ...
      .filteredBy( formData.aircraftCategoryId.flatMap{x => if(x>0L) Some(x) else None} )( _.aircraftCategoryId.getOrElse(0L) === _ )
  }
}