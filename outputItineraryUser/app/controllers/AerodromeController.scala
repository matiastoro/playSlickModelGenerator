package controllers
import models._
import models.extensions._
import play.api._
import play.api.db.slick._
import play.api.db.slick.Config.driver.simple._
import play.api.data._
import play.api.data.Forms._
import play.api.mvc.Controller
import play.api.Play.current
import play.api.mvc.BodyParsers._
import play.api.libs.json.Json
import play.api.libs.json.Json._
import com.roundeights.hasher.Implicits._
import com.github.nscala_time.time.Imports._
import play.api.libs.concurrent.Akka
import models._
import models.extensions._
import org.joda.time.{DateTimeZone, DateTime}
import play.api.i18n.Messages
object AerodromeController extends Controller with Autorizacion {
implicit val jsonFormat = Json.format[Aerodrome]

/*
GET         /aerodrome/                  controllers.AerodromeController.index(page: Int = 1, pageLength: Int = 20)
GET         /aerodrome/show/:id          controllers.AerodromeController.show(id: Long)
GET         /aerodrome/delete/:id          controllers.AerodromeController.delete(id: Long)
POST        /aerodrome/save              controllers.AerodromeController.save()
POST        /aerodrome/update/:id        controllers.AerodromeController.update(id: Long)
GET         /aerodrome/options             controllers.AerodromeController.options()
GET         /aerodrome/:page/:pageLength controllers.AerodromeController.index(page: Int, pageLength: Int)
GET         /aerodrome/:page             controllers.AerodromeController.index(page: Int, pageLength: Int = 20)
*/
  def index(page: Int = 1, pageLength: Int = 20) = conUsuarioDB{ user =>  implicit request =>
    val pagination = AerodromeQuery.paginate(AerodromeQuery.allQuery,pageLength,page)
    Ok(Json.toJson(Json.obj("results" -> pagination.results, "count" -> pagination.count, "page" -> pagination.page, "pageLength" -> pagination.pageLength)))
  }

  def show(id: Long) = conUsuarioDB{ user =>  implicit request =>
    objectResponse(id)
  }
  val form = AerodromeForm.form

  def objectResponse(id: Long)(implicit session: Session) = {
    AerodromeQuery.byId(id).map{ aerodrome =>
      Ok(Json.obj("obj" -> aerodrome.toJson))
    }.getOrElse(NotFound)
  }


  def save() = conUsuarioDB{ user =>  implicit request =>

    form.bindFromRequest.fold(
      formWithErrors => {
    
        Ok(Json.obj("errors" -> formWithErrors.errorsAsJson))
      }, formData => {
        val id = formData.insert(formData.obj)
        objectResponse(id)
      }
    )
  }
  

  def delete(id: Long) = conUsuarioDB{ user =>  implicit request =>
    AerodromeQuery.byId(id).map{ aerodrome =>
      AerodromeQuery.delete(aerodrome)
      Ok("ok")
    }.getOrElse(NotFound)
  }

  def update(id: Long) = conUsuarioDB{ user =>  implicit request =>
    AerodromeQuery.byId(id).map{ aerodrome =>
      form.bindFromRequest.fold(
        formWithErrors => {
     
          Ok(Json.obj("errors" -> formWithErrors.errorsAsJson))
        }, formData => {
          formData.update(formData.obj.copy(id = aerodrome.id)).map{ id =>
            objectResponse(id)
          }.getOrElse(NotFound)
        }
      )
    }.getOrElse(NotFound)
  }
}