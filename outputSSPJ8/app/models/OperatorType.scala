...
case class OperatorTypeFilter(id: Option[Long] = None,
                              operatorType: Option[String] = None,
                              operatorTypeId: Option[Long] = None)

object OperatorTypeFilter {
  val tupled = (this.apply _).tupled
}


object OperatorTypeFilterForm{
  
  val filterForm = Form(
            mapping(
      "id" -> optional(longNumber),
      "operatorType" -> optional(nonEmptyText),
      "operatorTypeId" -> optional(longNumber)
    )/*(OperatorTypeFilter.apply)(OperatorTypeFilter.unapply)*/
    ((id,operatorType,operatorTypeId) => {
      OperatorTypeFilter(id, operatorType, operatorTypeId)
    })((formData: OperatorTypeFilter) => {
      Some((formData.id, formData.operatorType, formData.operatorTypeId))
    })
  )
}
