...
case class HelicopterLandingAreaSurfaceTypeFilter(id: Option[Long] = None,
                                                  surfaceType: Option[String] = None)

object HelicopterLandingAreaSurfaceTypeFilter {
  val tupled = (this.apply _).tupled
}


object HelicopterLandingAreaSurfaceTypeFilterForm{
  
  val filterForm = Form(
            mapping(
      "id" -> optional(longNumber),
      "surfaceType" -> optional(nonEmptyText)
    )/*(HelicopterLandingAreaSurfaceTypeFilter.apply)(HelicopterLandingAreaSurfaceTypeFilter.unapply)*/
    ((id,surfaceType) => {
      HelicopterLandingAreaSurfaceTypeFilter(id, surfaceType)
    })((formData: HelicopterLandingAreaSurfaceTypeFilter) => {
      Some((formData.id, formData.surfaceType))
    })
  )
}
