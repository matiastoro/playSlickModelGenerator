...
class RunwayRepository @Inject() (dbConfigProvider: DatabaseConfigProvider, runway_surface_typeRepo: RunwaySurfaceTypeRepository, aerodromeRepo: AerodromeRepository)(implicit ec: ExecutionContext) extends DatabaseClient[Runway] with RunwayQuery{
  ...
  */
  ...
  override val sortMap = Map[String, SortMapType](
    ...
    "runway" -> {(q,s) => q.sortBy(_.runway)(s)},
  ...
  */
  ...
  def filter(formData: RunwayFilter) = {
    all
      .filteredBy( formData.id.flatMap{x => if(x>0L) Some(x) else None} )( _.id === _ )
      .filteredBy(formData.runway)((x,y) => x.runway.toUpperCase like ('%'+y.toUpperCase+'%'))
      .filteredBy( formData.runwaySurfaceTypeId.collect{case x if x>0L => x} )( _.runwaySurfaceTypeId === _ )
