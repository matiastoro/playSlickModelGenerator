...
*/
  ...
  def save() = Action{ implicit request =>
    ...
    form.bindFromRequest.fold(
      formWithErrors => {
        ...
        Redirect(controllers.routes.BranchController.show(id)).flashing("success" -> Messages("save.success"))
      }
    )
  }
  ...
  def delete(id: Long) = Action{ implicit request =>
    BranchQuery.byId(id).map{ branch =>
      ...
      Redirect(controllers.routes.BranchController.index(1,20)).flashing("success" -> Messages("delete.success"))
    }.getOrElse(NotFound)
  }
        ...
        formWithErrors => {
         val company = CompanyQuery.getAll
          ...
          formData.update(formData.obj.copy(id = branch.id, createdAt = branch.createdAt)).map{ id =>
            Redirect(controllers.routes.BranchController.show(branch.id.get)).flashing("success" -> Messages("save.success"))
          }.getOrElse(NotFound)
        }
      )
    }.getOrElse(NotFound)
