...
class TrafficTypeRepository @Inject() (dbConfigProvider: DatabaseConfigProvider)(implicit ec: ExecutionContext) extends DatabaseClient[TrafficType] with TrafficTypeQuery{
  ...
  */
  ...
  override val sortMap = Map[String, SortMapType](
    ...
    "trafficType" -> {(q,s) => q.sortBy(_.trafficType)(s)}
  )
  ...
  */
  ...
  def filter(formData: TrafficTypeFilter) = {
    all
      .filteredBy( formData.id.flatMap{x => if(x>0L) Some(x) else None} )( _.id === _ )
      .filteredBy(formData.trafficType)((x,y) => x.trafficType.toUpperCase like ('%'+y.toUpperCase+'%'))
  }
}