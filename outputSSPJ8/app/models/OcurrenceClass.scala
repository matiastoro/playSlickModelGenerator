...
case class OcurrenceClassFilter(id: Option[Long] = None,
                                ocurrenceClass: Option[String] = None)

object OcurrenceClassFilter {
  val tupled = (this.apply _).tupled
}


object OcurrenceClassFilterForm{
  
  val filterForm = Form(
            mapping(
      "id" -> optional(longNumber),
      "ocurrenceClass" -> optional(nonEmptyText)
    )/*(OcurrenceClassFilter.apply)(OcurrenceClassFilter.unapply)*/
    ((id,ocurrenceClass) => {
      OcurrenceClassFilter(id, ocurrenceClass)
    })((formData: OcurrenceClassFilter) => {
      Some((formData.id, formData.ocurrenceClass))
    })
  )
}
