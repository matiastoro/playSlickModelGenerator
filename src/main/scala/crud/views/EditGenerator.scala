...
case class EditGenerator(table: Table) extends CodeGenerator{
  ...
  def generate: String = {
    ...
    val html = """@(frm: Form[models."""+formClass+"""]"""+args+""", """+table.objName+""": models."""+table.className+""")(implicit flash: Flash, lang: Lang, session: Session, context: ApplicationContext, request: Request[AnyContent])
@import helper._
