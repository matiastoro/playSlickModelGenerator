...
case class AerodromeLocationFilter(id: Option[Long] = None,
                                   location: Option[String] = None)

object AerodromeLocationFilter {
  val tupled = (this.apply _).tupled
}


object AerodromeLocationFilterForm{
  
  val filterForm = Form(
            mapping(
      "id" -> optional(longNumber),
      "location" -> optional(nonEmptyText)
    )/*(AerodromeLocationFilter.apply)(AerodromeLocationFilter.unapply)*/
    ((id,location) => {
      AerodromeLocationFilter(id, location)
    })((formData: AerodromeLocationFilter) => {
      Some((formData.id, formData.location))
    })
  )
}
