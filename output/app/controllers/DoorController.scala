...
*/
  ...
  def save() = Action{ implicit request =>
    ...
    form.bindFromRequest.fold(
      formWithErrors => {
        ...
        Redirect(controllers.routes.DoorController.show(id)).flashing("success" -> Messages("save.success"))
      }
    )
  }
  ...
  def delete(id: Long) = Action{ implicit request =>
    DoorQuery.byId(id).map{ door =>
      ...
      Redirect(controllers.routes.DoorController.index(1,20)).flashing("success" -> Messages("delete.success"))
    }.getOrElse(NotFound)
  }
        ...
        formWithErrors => {
         val branch = BranchQuery.getAll
          ...
          formData.update(formData.obj.copy(id = door.id)).map{ id =>
            Redirect(controllers.routes.DoorController.show(door.id.get)).flashing("success" -> Messages("save.success"))
          }.getOrElse(NotFound)
        }
      )
    }.getOrElse(NotFound)
