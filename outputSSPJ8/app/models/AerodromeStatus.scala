...
case class AerodromeStatusFilter(id: Option[Long] = None,
                                 status: Option[String] = None)

object AerodromeStatusFilter {
  val tupled = (this.apply _).tupled
}


object AerodromeStatusFilterForm{
  
  val filterForm = Form(
            mapping(
      "id" -> optional(longNumber),
      "status" -> optional(nonEmptyText)
    )/*(AerodromeStatusFilter.apply)(AerodromeStatusFilter.unapply)*/
    ((id,status) => {
      AerodromeStatusFilter(id, status)
    })((formData: AerodromeStatusFilter) => {
      Some((formData.id, formData.status))
    })
  )
}
